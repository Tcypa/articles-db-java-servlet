...
private static final String JSON_FILE_PATH ="userdata.json";
    ...
    private boolean isUserAlreadyRegistered(List<User> users, String username) {
        for (User user : users) {
    ...
    private boolean isUserPassValid(List<User> users, String username,String password) {
        for (User user : users) {
            if (username.equals(user.getUsername())) {
                return true;
            }
